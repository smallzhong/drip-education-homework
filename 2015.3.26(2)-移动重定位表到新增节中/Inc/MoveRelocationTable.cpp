...
int main()
{
	...
	CreateNewSection(pFileBuffer, pDosHeader, pNTHeader, pSectionHeader,
		file_size, 0x1000, t_NameOfNewSectionHeader);
	//realloc(pFileBuffer, file_size + 0x2000);

	MoveRelocationTable(pFileBuffer, pDosHeader, pNTHeader, pSectionHeader, file_size + 0x100);
	//SaveToFile("e:\\123mydll.dll", file_size + 0x2000, pFileBuffer);
	FILE* fp;
	fp = fopen("e:\\aaaamydll.dll", "wb");
	if (fp == NULL)
		EXIT_ERROR("fp == MULL");
	fwrite(pFileBuffer, file_size + 0x2000, 1, fp);
	fclose(fp);
	//free(pFileBuffer);
	return 0;
}
	...
	PIMAGE_SECTION_HEADER pSectionHeader, DWORD add_to_location)
{
	...
	RVA_TO_FOA(pFileBuffer, pDosHeader, pNTHeader, pSectionHeader, fore_relocation_table_addr_RVA, &fore_relocation_table_addr_FOA);
	PIMAGE_BASE_RELOCATION pForeBaseRelocation = (PIMAGE_BASE_RELOCATION)((DWORD)pFileBuffer + fore_relocation_table_addr_FOA);
	assert(pForeBaseRelocation);

	// 开始遍历重定向表，并memcpy到新增的节中
	PIMAGE_BASE_RELOCATION pAfterBaseRelocation = (PIMAGE_BASE_RELOCATION)((DWORD)pFileBuffer + add_to_location);
	while (1)
	{
		if (pForeBaseRelocation->SizeOfBlock == 0/*&& pForeBaseRelocation->VirtualAddress == 0*/)
			break;
		...
		for (int i = 0; i < num_of_addr; i++)
		{
			if ((t_pAddr[i] & 0x3000) == 0x3000) // 判断高三位是否为0011
				cout << hex
				<< (t_pAddr[i] & 0xfff) + pForeBaseRelocation->VirtualAddress
				<< endl;
			else
		...
		memcpy(pAfterBaseRelocation, pForeBaseRelocation, pForeBaseRelocation->SizeOfBlock); // 根据SizeOfBlock将一整块复制过去
		pForeBaseRelocation = (PIMAGE_BASE_RELOCATION)((DWORD)pForeBaseRelocation + pForeBaseRelocation->SizeOfBlock);
		pAfterBaseRelocation = (PIMAGE_BASE_RELOCATION)((DWORD)pAfterBaseRelocation + pAfterBaseRelocation->SizeOfBlock);
	}
	memset(pAfterBaseRelocation, 0, 8); // 在最后补上8个0

	DWORD t_NewRelocationTableAddr_RVA = 0;
	FOA_TO_RVA(pFileBuffer, pDosHeader, pNTHeader, pSectionHeader, add_to_location, &t_NewRelocationTableAddr_RVA);
	pNTHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].VirtualAddress = t_NewRelocationTableAddr_RVA;
}
	...
	LPSTR NameOfNewSetionHeader)
{
	...
	memset(pNewSectionHeader + 1, 0, sizeof(IMAGE_SECTION_HEADER)); // 在添加的节后面填充0
	...
	//memset((PBYTE)pFileBuffer + size_of_new_section, 0, pNTHeader->OptionalHeader.FileAlignment);
	// 再增加一个文件对齐大小的0
...
PIMAGE_NT_HEADERS32
GetNTHeader(LPVOID pFileBuffer, PIMAGE_DOS_HEADER* pDosHeader)
{
